...
public:
	...
	void allBfs(string label) {
		...
		for (set<string>::iterator it = landmarkByLabel.begin(); it != landmarkByLabel.end(); it++) {
			...
			reachabilityByLabelAndLandmark[*it] = 0;
	...
	void bfs(string start, string label) {
		map<string, map<string, int> > labelBasedLandmark;
	...
	bool isReachable(string start, string end, string label, int distance) {
		...
		if (isLandmarkVertex(end, label)) {
			return (landmarkReachable[label][end][start] <= distance);
		}
		...
		while((!q.empty()) && (!revQ.empty())) {
			...
			for(int i = 0; i < backEdges.size(); i++) {
				...
				if (isLandmarkVertex(backEdges[i].end,label)) {
					if ((landmarkReachable[label][backEdges[i].end][start] + revNodeDist[backEdges[i].end]) <= distance) {
						return true;
					}
				}
				revQ.push(backEdges[i].end);
...
Graph getGraph(int randomGraph) {
	if (randomGraph > 0) {
		...
		printf("Enter labels now\n");
		for(int i = 0; i < numLabels; i++) {
			string curr;
			cin >> curr;
			labels.push_back(curr);
		}
		printf("Enter edges in start, end, label format\n");
